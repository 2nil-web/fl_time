...
size_t read_rmp_size(FILE *file)
{
  ...
  if (!check_read(file, expected_header, sizeof(expected_header)))
  {
    return 0;
  }
  ...
  if (!check_read(file, expected_strings, sizeof(expected_strings)))
  {
    return 0;
  }
  ...
  if (!check_read(file, expected_layer_header, sizeof(expected_layer_header)))
  {
    return 0;
  }

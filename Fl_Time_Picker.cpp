...
int Fl_Time_Picker::col_x(int c) const {
    int x = 0;
    for (int i = 0; i < c; i++) x += col_w_[i];
    return x;
}

int Fl_Time_Picker::col_at(int mx) const {
    for (int c = 0; c < 4; c++)
        if (mx < col_x(c) + col_w_[c]) return c;
    return 3;
}
...
void Fl_Time_Picker::draw() {
    ...
    for (int c = 0; c < 4; c++) draw_col(c, col_x(c));
...
int Fl_Time_Picker::handle(int ev) {
    ...
    case FL_KEYDOWN:
        if (Fl::event_key() == FL_Escape) { cancel_and_hide(); return 1; }
        if (Fl::event_key() == FL_Enter)  { accept_and_hide(); return 1; }
        break;
    }

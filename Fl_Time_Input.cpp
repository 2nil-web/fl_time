...
Fl_Time_Input::Fl_Time_Input(int X, int Y, int W, int H, const char* lbl)
  : Fl_Group(X, Y, W, H, lbl),
    h_(12), m_(0), s_(0), pm_(false),
    active_seg_(SEG_NONE),
    picker_(nullptr),
    user_cb_(nullptr), user_data_(nullptr)
{
    end();
    box(FL_NO_BOX);
}
...
void Fl_Time_Input::fire_callback() {
    if (user_cb_) user_cb_(this, user_data_);
}
...
void Fl_Time_Input::resize(int X, int Y, int W, int H) {
    ...
    int pw, ph;
    preferred_size(pw, ph);
    (void)W; (void)H;   // dimensions demandées ignorées
    Fl_Group::resize(X, Y, pw, ph);
}
...
void Fl_Time_Input::compute_rects() {
    ...
    r_.cw = clock_w; r_.ch = rh; r_.cx = x() + w() - clock_w; r_.cy = ry;
    r_.sw = spin_w;  r_.sh = rh; r_.sx = r_.cx - spin_w;       r_.sy = ry;
    r_.fx = x();     r_.fy = ry; r_.fw = r_.sx - x();           r_.fh = rh;

    int pad = fm_.pad;
    int px  = r_.fx + pad;

    r_.seg_x[SEG_HOUR] = px; r_.seg_w[SEG_HOUR] = fm_.cw_num;  px += fm_.cw_num  + fm_.sep_w;
    r_.seg_x[SEG_MIN]  = px; r_.seg_w[SEG_MIN]  = fm_.cw_num;  px += fm_.cw_num  + fm_.sep_w;
    r_.seg_x[SEG_SEC]  = px; r_.seg_w[SEG_SEC]  = fm_.cw_num;  px += fm_.cw_num  + pad;
    r_.seg_x[SEG_AMPM] = px; r_.seg_w[SEG_AMPM] = fm_.cw_ampm;
}

// ---- Dessin ------------------------------------------------

void Fl_Time_Input::draw_segment(const char* txt,
...
void Fl_Time_Input::draw() {
    ...
    draw_segment(pm_ ? "PM" : "AM",
                 r_.seg_x[SEG_AMPM], r_.fy, r_.seg_w[SEG_AMPM], r_.fh,
                 active_seg_ == SEG_AMPM);
...
void Fl_Time_Input::increment(int delta) {
    switch (active_seg_) {
    case SEG_HOUR: h_ += delta; if (h_ > 12) h_ = 1;  if (h_ < 1) h_ = 12; break;
    case SEG_MIN:  m_ = (m_ + delta + 60) % 60; break;
    case SEG_SEC:  s_ = (s_ + delta + 60) % 60; break;
    case SEG_AMPM: pm_ = !pm_; break;
    default: break;
    }
    redraw();
    fire_callback();
...
int Fl_Time_Input::handle(int ev) {
    ...
    case FL_PUSH: {
        ...
        // Bouton horloge
        if (mx >= r_.cx && mx <= r_.cx + r_.cw &&
            my >= r_.cy && my <= r_.cy + r_.ch) {
            open_picker();
            return 1;
        }
        // Bouton spin
        if (mx >= r_.sx && mx <= r_.sx + r_.sw &&
            my >= r_.sy && my <= r_.sy + r_.sh) {
            if (active_seg_ == SEG_NONE) active_seg_ = SEG_HOUR;
    ...
    case FL_KEYDOWN: {
        ...
        if (key == FL_Up)   { if (active_seg_ == SEG_NONE) active_seg_ = SEG_HOUR; increment(1);  return 1; }
        if (key == FL_Down) { if (active_seg_ == SEG_NONE) active_seg_ = SEG_HOUR; increment(-1); return 1; }
        if (key == FL_Tab && !Fl::event_shift()) { next_segment(); return 1; }
        if (key == FL_Tab &&  Fl::event_shift()) { prev_segment(); return 1; }
        if (key == FL_Right) { next_segment(); return 1; }
        if (key == FL_Left)  { prev_segment(); return 1; }
        if (key >= '0' && key <= '9') {
            int d = key - '0';
            if (active_seg_ == SEG_HOUR) h_ = (d == 0 ? 12 : d);
            if (active_seg_ == SEG_MIN)  m_ = d * 10;
            if (active_seg_ == SEG_SEC)  s_ = d * 10;
            redraw();
            fire_callback();
            return 1;
        }
        if (key == 'a' || key == 'A') { pm_ = false; redraw(); fire_callback(); return 1; }
        if (key == 'p' || key == 'P') { pm_ = true;  redraw(); fire_callback(); return 1; }
        break;
    }
